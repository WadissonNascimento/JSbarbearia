...
test("customer profile email changes require code verification and do not verify phones", () => {
  ...
  assert.match(form, /n[aã]o precisa de verifica[cç][aã]o por SMS/);
...
test("barber actions use the authenticated barber id instead of trusting form ids", () => {
  ...
  assert.match(actions, /Cliente selecionado n[aã]o pertence a sua base/);
  assert.match(actions, /Cliente n[aã]o vinculado a este barbeiro/);
...
test("custom domain readiness remains read-only for shop domains", () => {
  ...
  assert.match(domainReadiness, /if \(!expectedIpv4s\.length\)/);
  ...
  assert.match(domainActivateScript, /server_name \$\{domain\}/);
  assert.match(domainActivateScript, /proxy_set_header Host \$host/);
