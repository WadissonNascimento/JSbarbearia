...
const originalPaymentsEnabled = process.env.VIP_ASAAS_PAYMENTS_ENABLED;
test.before(() => { process.env.VIP_ASAAS_PAYMENTS_ENABLED = "true"; });
test.after(() => {
  if (originalPaymentsEnabled === undefined) delete process.env.VIP_ASAAS_PAYMENTS_ENABLED;
  else process.env.VIP_ASAAS_PAYMENTS_ENABLED = originalPaymentsEnabled;
});
