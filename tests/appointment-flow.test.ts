...
const integrationOptions = {
  skip: process.env.APPOINTMENT_INTEGRATION_TEST !== "1",
};
...
test("customer can book and conclude an appointment", integrationOptions, async () => {
...
test("manual fit-in bypasses availability without changing financial snapshots", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("customer cannot create overlapping appointment for the same barber", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("customer cannot create concurrent appointment for the same barber and time", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("barber can edit an open appointment without revalidating an unchanged slot", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("barber can record a longer performed service without moving the appointment slot", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("booking availability respects recurring blocks and ignores cancelled appointments", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("customer can reschedule and old slot becomes available", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("customer can reserve extras and stock returns on cancellation", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("barber reviews extras before completing and payout uses delivered items only", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("admin can change a completed appointment to no-show without double-returning extras", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
...
test("admin can reopen a completed appointment and barber remains blocked", integrationOptions, async () => {
  const { db, runId, cleanup } = await setupDatabase();
