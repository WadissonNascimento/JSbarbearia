...
export default function VipPaymentMethodFields({
  ...
  return (
    <fieldset disabled={disabled} className="min-w-0">
      <legend className="mb-2 text-xs font-bold text-[#f5f5f5] sm:mb-3 sm:text-sm">Como você quer pagar?</legend>
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        {methods.map(({ value: method, title, shortTitle, description, icon: Icon }) => (
          <label key={method} title={title} className={`relative flex min-h-[78px] min-w-0 cursor-pointer flex-col items-center justify-center gap-1 rounded-xl border px-1.5 py-2 text-center transition focus-within:ring-2 focus-within:ring-[#dedede] sm:min-h-0 sm:items-start sm:justify-start sm:gap-2 sm:p-4 sm:text-left ${value === method ? "border-[#dedede] bg-[#dedede]/10" : "border-white/15 bg-black/20 hover:border-white/35"}`}>
            ...
            <span className="min-w-0">
              <Icon className="mx-auto mb-1 h-5 w-5 text-[#dedede] sm:mx-0 sm:mb-2" aria-hidden="true" />
              <span className="block truncate text-xs font-bold text-[#f5f5f5] sm:text-sm"><span className="sm:hidden">{shortTitle}</span><span className="hidden sm:inline">{title}</span></span>
              <span className="mt-1 hidden text-xs leading-5 text-[#b4b4b4] sm:block">{description}</span>
            </span>
          </label>
        ))}
      </div>
