...
export default function VipSubscribeButton({ planCode, signedIn, enrollmentOpen }: { planCode: string; signedIn: boolean; enrollmentOpen: boolean }) {
  ...
  if (!signedIn) {
    return <Link href="/login?redirectTo=%2Fplanos" className="mt-6 inline-flex min-h-12 w-full items-center justify-center rounded-lg bg-[#e5e5e5] px-5 text-sm font-black text-[#080808]">Entrar para assinar</Link>;
  }
  return (
    <form action={formAction} className="mt-6 grid gap-2">
      <input type="hidden" name="planCode" value={planCode} />
      <label className="grid gap-2 text-sm font-bold text-[#f5f5f5]">
        CPF ou CNPJ
        <input name="cpfCnpj" inputMode="numeric" required maxLength={18} className="min-h-12 rounded-xl border border-white/15 bg-black/30 px-4 text-white outline-none focus:border-[#dedede]" />
      </label>
      ...
      <p className="text-xs leading-5 text-[#c2c2c2]">Vencimento no 5º dia útil. Se a mensalidade atual já estiver vencida, ela será emitida para pagamento agora. No cartão, a cobrança vencida pode ser processada ao confirmar.</p>
      <button type="submit" disabled={pending || !billingType} className="min-h-12 rounded-lg bg-[#e5e5e5] px-5 text-sm font-black text-[#080808] disabled:opacity-60">{pending ? "Criando..." : "Assinar plano"}</button>
