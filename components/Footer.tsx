...
export default function Footer({
  ...
  const effectiveLocationUrl =
    locationUrl ||
    (addressLine && !/sob consulta/i.test(addressLine)
  ...
  return (
    <footer className="relative mt-14 border-t border-white/10 bg-[#080808] text-white">
      <div className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(circle_at_bottom,_rgba(200,200,200,0.16),_transparent_36%)]" />
...
function UnavailableFooterAction({
  children,
  label,
}: {
  ...
  return (
    <span
      aria-label={label}
      title={label}
      className="flex h-11 w-11 cursor-not-allowed items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-[var(--brand-strong)] opacity-55"
    >
      {children}
    </span>
  );
