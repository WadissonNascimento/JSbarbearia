...
function readConfiguredVipPrice(value: string | undefined) {
...
export const VIP_PLAN_DEFINITIONS = [
  {
    code: "CORTE",
    name: "Bronze",
    price: readConfiguredVipPrice(process.env.JS_VIP_CORTE_PRICE),
    tokensPerCycle: 4,
  },
  {
    code: "CORTE_SOBRANCELHA",
    name: "Prata",
    price: readConfiguredVipPrice(process.env.JS_VIP_CORTE_SOBRANCELHA_PRICE),
    tokensPerCycle: 4,
  },
  {
    code: "CORTE_BARBA_SOBRANCELHA",
    name: "Ouro",
    price: readConfiguredVipPrice(process.env.JS_VIP_COMPLETO_PRICE),
    tokensPerCycle: 4,
  },
] as const;
...
export async function ensureVipPlansForShop(db: VipDb, shopId: string) {
  const configuredPlans = VIP_PLAN_DEFINITIONS.flatMap((plan) =>
    plan.price === null ? [] : [{ ...plan, price: plan.price }]
  );

  if (configuredPlans.length === VIP_PLAN_DEFINITIONS.length) {
    await Promise.all(
